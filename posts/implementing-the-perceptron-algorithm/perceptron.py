...
class Perceptron(LinearModel):
    ...
    def loss(self, X, y):
        ...
        y_ = 2 * y - 1
        scores = self.score(X)
        misclassified = (scores * y_ <= 0)
        return torch.mean(misclassified.float())

    def grad(self, X, y, lr):
        y_ = 2 * y - 1 
        scores = self.score(X)
        misclassified = (scores * y_ < 0).float()
        grad = -lr * torch.matmul((misclassified * y_).unsqueeze(0), X).squeeze(0) / X.size(0)
        return grad
...
class PerceptronOptimizer:
    ...
    def step(self, X, y, lr = 1.0):
        """
        Compute one step of the perceptron update using the feature matrix X 
        and target vector y. 
        """
        self.model.loss(X, y)
        grad = self.model.grad(X, y, lr)
        self.model.w -= grad